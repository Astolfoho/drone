...
public:
    ...
    void start(char *ssid, char *password);
...
int WifiManager::_s_retry_num = 0;
...
static void _event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    {
        if (WifiManager::_s_retry_num < 3)
        {
            esp_wifi_connect();
            WifiManager::_s_retry_num++;
...
void WifiManager::start(char *ssid, char *password)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_init(&iconfig));
    ...
    memcpy(wifi_config.sta.password,password,20);
